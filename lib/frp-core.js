// Delt modul: henting, filtrering og klassifisering av Frp-utspill i norske
// medier. Brukt av /api/frp-nyheter (JSON, til Innsikt-siden og Frp-utspill-siden)
// og /api/frp-rss (RSS 2.0 XML, til «Følg»-siden og eksterne RSS-lesere).

import { decodeGoogleNewsUrl, parseRssItems } from './gnews.js';
...
/**
 * Henter, dedupliserer, klassifiserer og filtrerer Frp-utspill i norske medier.
 * @param {{omraade?: string, tema?: string, limit?: number, decodeLinks?: boolean}} opts
 * @returns {Promise<{items: object[], omraade: string, tema: string|null}>}
 */
export async function getFrpNyheter(opts = {}) {
  const omraade = (opts.omraade || 'alle').toLowerCase().trim();
  const filterTema = (opts.tema || '').toLowerCase().trim();
  const limit = Math.min(Math.max(opts.limit || 40, 1), 60);
  const decodeLinks = opts.decodeLinks !== false;
  ...
  return { items, omraade: omraade || 'alle', tema: filterTema || null };
}
